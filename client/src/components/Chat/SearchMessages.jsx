...
function SearchMessages() {
  ...
  useEffect(() => {
    if(searchTerm){
      setSearchedMessages(messages.filter(msg => msg.type === "text" && msg.message.includes(searchTerm)))
    }else{
      setSearchedMessages([])
    }
  }, [searchTerm]);
      ...
      <div className="overflow-auto custom-scrollbar h-full">
        <div className="flex items-center flex-col w-full">
          <div className="flex px-5 items-center gap-3 h-14 w-full">
            <div className="bg-panel-header-background flex items-center gap-5 px-3 py-1 rounded-lg flex-row w-full">
              <div>
