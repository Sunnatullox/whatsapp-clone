...
function ChatLIstItem({ data, isContactPage = false }) {
  ...
  const handleContactsClick = (userData) => {
    ...
    if (!isContactPage) {
      ...
      dispatch({ type: reducerCases.CHANGE_CURRENT_CHAT_USER, user: userData });
  ...
  return (
    <div
      ...
      onClick={() => handleContactsClick(data)}
    >
      <div className="min-w-fit px-5 pt-3 pb-1 relative">
        <Avatar type="lg" image={data?.profilePicture} />
        {onlineUsers.includes(data.id) &&(
          <span className="flex w-3 h-3 me-3 bg-blue-600 rounded-full mt-[-15px] right-2 absolute"></span>
        )}
      </div>
